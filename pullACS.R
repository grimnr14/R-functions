...
crowd<-c(
  "B11017_001","B11017_002",
  "B26108_001","B26108_002",#all homes w disabilities
  "B26108_013","B26108_014",#group quarters with disability
  "B25002_001",#housing units
  "B25003_002","B25003_003"#owner vs renter occupied
)
...
pullACS<-function(geography="county",variables=c("B01001_001"),state=state.abb,geometry=T,year=2019,impute=F,impute.method="rpart",calc="none",calc.adi=F,survey="acs5"){
  ...
  if(calc=="housing"){
    vars<-data.frame(
      variable=c("housingvalue","mortgage","mortgagecost","mortgagetax","mortgageval","rentgross","occupyown","occupy",
                 "units","unitsownpop","vehicle","fuelheat","kitchen","plumbing","rooms",
                 "multigeneration",
                 "unitsperperson",
                 "ownervsrenter",
                 "pcthouseholdswithmorethanoneperroom"),
      numerator=c("B25077_001","B25027_002","B25089_002","B25103_001","B25082_002","B25064_001","B25003_002","B25002_002",
                  "B25032_001","B25008_002","B25046_001","B25040_002+B25040_003+B25040_004+B25040_005+B25040_008","B25051_002","B25047_002","B25018_001",
                  "B10063_002",
                  "B25002_001",
                  "B25010_002/B25022_002",
                  "B25014_005+B25014_006+B25014_007+B25014_011+B25014_012+B25014_013"),
      denominator=c(NA,"B25027_001","B25089_001",NA,"B25082_001",NA,"B25003_001","B25002_001","B01001_001","B25008_001","B25045_001","B25040_001","B25051_001","B25047_001",NA,
                    "B10063_001",
                    "B01001_001",
                    "B25010_003/B25022_003",
                    "B25014_001")
    )
    ...
    ex$multigeneration<-100*ex$multigeneration
    ex$ownervsrenter<-ifelse(is.infinite(ex$ownervsrenter),max(na.omit(ex[!is.infinite(ex$ownervsrenter),]$ownervsrenter)),ex$ownervsrenter)
    ex$pcthouseholdswithmorethanoneperroom<-100*ex$pcthouseholdswithmorethanoneperroom

    ex$NAME<-as.character(1:nrow(ex))
    out<-makeIndex(x=ex[complete.cases(ex),],geoid="GEOID",nfactors=4,state=NULL,
                   set.factor=list(financial=c("housingvalue",
                                               "mortgage",
                                               "mortgagecost",
                                               "mortgagetax",
                                               "mortgageval",
                                               "rentgross"),
                                   ownership=c("occupyown",
                                               "unitsownpop",
                                               "ownervsrenter"),
                                   building_quality=c("fuelheat",
                                                      "kitchen",
                                                      "plumbing",
                                                      "occupy"),
                                   crowding=c("multigeneration",
                                              "pcthouseholdswithmorethanoneperroom",
                                              "unitsperperson",
                                              "rooms")
                   ))
    ...
    names(housing)<-c("GEOID",vars$variable,"housing_score","housing_rnk",
                      "financial_score","ownership_score","building_quality_score","crowding_score",
                      #"geometry",#toggle off from zcta...
                      "housing_state_rnk")
    if(calc.adi==T){
      housing<-housing[,!names(housing) %in% c("pcthouseholdswithmorethanoneperroom")]
    }
